...
import multiprocessing # <--- 新增：用于探测系统真实的物理核心数
...
try:
    ...
    pass  # 如果遇到无控制台模式（GUI），直接静默跳过，绝不报错
...
class FFmpegOrchestrator:
    def __init__(self):
        ...
        if getattr(sys, 'frozen', False):
            # 运行打包后的 exe 时，获取 exe 所在的目录
            app_path = os.path.dirname(sys.executable)
        else:
            # 运行 python 源码时，获取脚本所在的目录
            app_path = os.path.dirname(os.path.abspath(__file__))

        self.ffmpeg_cmd = os.path.join(app_path, "ffmpeg.exe")
        self.ffprobe_cmd = os.path.join(app_path, "ffprobe.exe")
        
        # 智能兜底：如果同级目录没放 ffmpeg，就尝试调用系统的
        if not os.path.exists(self.ffmpeg_cmd): self.ffmpeg_cmd = "ffmpeg"
        if not os.path.exists(self.ffprobe_cmd): self.ffprobe_cmd = "ffprobe"
    ...
    def build_video_compress_command(self, input_file, output_file, video_category, quality_level, target_vcodec, use_hw=True, ext_audio=None, ffmpeg_threads=0):
        """构建全场景差异化高保真画质与压缩的独立算法参数链"""
        cmd = list()
        
        # ================== 提速核心 1：加入硬件解码器 ==================
        cmd.extend((self.ffmpeg_cmd, "-y"))
        if use_hw:
            cmd.extend(("-hwaccel", "auto"))  # 允许显卡自动接管解码，大幅降低 CPU 负担

        # 外部音轨合并与映射逻辑 (庞大无损音轨自动压缩)
        if ext_audio and ext_audio != "none":
            cmd.extend(("-i", input_file, "-i", ext_audio))
        ...
        if use_hw and len(self.available_hw_encoders) > 0:
            ...
            if target_vcodec == "av1":
                ...
                cpu_args = list()
                # 🚀 提速：将 preset slow 替换为 medium
                cpu_args.extend(("-c:v", "libx265", "-pix_fmt", "yuv420p10le", "-preset", "medium", "-crf", cq_val))
                if video_category == "1":
                ...
                # 🚀 提速：将 preset slow 替换为 medium
                cmd.extend(("-c:v", "libx264", "-preset", "medium", "-crf", cq_val, "-tune", tune_arg))
        ...
        return cmd
    def build_format_factory_command(self, input_file, output_file, format_type, compress_level=None, use_hw=True, target_vcodec="h264", ext_audio=None, a_codec="未知", v_codec="未知", ffmpeg_threads=0):
        ...
        # ================== 提速核心 1：加入硬件解码器 ==================
        cmd.extend((self.ffmpeg_cmd, "-y"))
        # 仅在非纯音频、非动图的情况下开启硬件解码
        if use_hw and format_type not in audio_formats and format_type not in image_formats:
        ...
        if format_type in audio_formats:
            ...
            if v_codec in ("无", "未知"):
                # 【底层防崩】：如果纯音频强行装入 MP4/MKV，绝对不能调用视频编码器！
                if compress_level:
                    audio_kbps = {"1": "320k", "2": "192k", "3": "128k", "4": "64k"}
                    target_br = audio_kbps.get(compress_level, "192k")
                    cmd.extend(("-c:a", "aac", "-b:a", target_br))
                else:
                    cmd.extend(("-c:a", "copy"))
                ...
                if compress_level:
                    ...
                    if use_hw and len(self.available_hw_encoders) > 0:
                        if "nvenc" in self.available_hw_encoders:
                            ...
                            cmd.extend(("-c:v", encoder, "-preset", "p4", "-rc", "vbr", "-cq", cq_val))
                        elif "qsv" in self.available_hw_encoders:
    ...
    def execute_with_fallback(self, cmd, input_file="", output_file="", out_dir="", **kwargs):
        # 【改动 1】删掉这里的 self._is_user_aborted = False，替换成下面这两行拦截器：
        if getattr(self, '_is_user_aborted', False):
            return False  # 只要被标记了终止，绝对拒绝再启动新的 Popen 进程
        ...
        if exit_code!= 0:
            if getattr(self, '_is_user_aborted', False): # <--- 新增这行：如果是人工终止，直接返回，绝对不触发容灾！
                return False
            ...
            # === 新增：无损拷贝 (Direct Stream Copy) 失败时的终极转码容灾机制 ===
            elif not is_video_compress and kwargs.get("compress_level") is None:
                self.print_guide("【触发容灾机制】目标容器拒绝直接封入原音视频流，自动热切换为【极高画质强制转码】重试！", "WARNING")
                
                # 强制将 kwargs 中的压缩挡位修改为 "1" (极高画质/高保真无损级)
                ...
                fallback_cmd = self.build_format_factory_command(
                    input_file, output_file,
                    format_type=kwargs.get("format_type"),
                    compress_level="1", # 启用转码
                    use_hw=kwargs.get("use_hw", True),  # 依然尝试用显卡加速
                    target_vcodec=kwargs.get("target_vcodec", "h264"), # 默认给它套个最稳的 H.264
                    ext_audio=kwargs.get("ext_audio"),
                    a_codec=kwargs.get("a_codec", "未知"),
                    v_codec=kwargs.get("v_codec", "未知")
                )
    ...
    def _ask_settings(self, input_path, choice, is_batch=False):
        ...
        if choice == "1":
            ...
            # 如果用户直接回车(空值)或者输入了乱码，强制锁定为默认的 2 (H.265)
            if not codec_opt or codec_opt not in ("1", "2", "3"):
                codec_opt = "2"
                
            target_vcodec = "hevc" if codec_opt == "2" else ("av1" if codec_opt == "3" else "h264")
...
# 新增 ffmpeg_threads=0 参数
    def _execute_worker(self, input_file, out_dir, settings, is_batch=False, ffmpeg_threads=0):
        # 【改动 2】在函数最开头加上排队任务拦截器
        if getattr(self, '_is_user_aborted', False):
            return
        ...
        if choice == "1":
            cmd = self.build_video_compress_command(
                ...
                ffmpeg_threads=ffmpeg_threads # <--- 传给构建器
            )
        ...
        while True:
            try:
                ...
                if is_multi_task:
                    ...
                    if max_workers > 1:
                        ffmpeg_threads = max(1, (total_cores - 1) // max_workers)
                        self.print_guide(f"系统算力分配: 侦测到 {total_cores} 个逻辑核心，已为每个并发任务严格限制 {ffmpeg_threads} 个处理线程以防性能内耗。", "INFO")

                # --- 2. 组装任务队列 ---
                tasks = []
                if batch_mode and is_multi_task:
                    first_file = next(iter(files))
                    settings = self._ask_settings(first_file, choice, is_batch=True)
                    for f in files:
                        tasks.append((f, settings))
                else:
                    for idx, f in enumerate(files):
                        if is_multi_task:
                            self.print_guide(f"\n>>> 正在配置独立任务 ({idx+1}/{len(files)}): {Path(f).name}", "INFO")
                ...
                if is_multi_task:
                    ...
                    self.batch_total_duration = sum(self.check_media_info(f).get("duration", 0) for f, _ in tasks)
                    self.batch_start_time = time.time()
                    self.file_progress_dict = {}
                    self.last_batch_percent = 0.0

                # --- 4. 引擎点火，执行统一并发 ---
                self.print_guide(f">>> [引擎启动] 正在以 {max_workers} 线程并发执行队列...\n", "SUCCESS")
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = []
                    for f_path, f_settings in tasks:
                        futures.append(executor.submit(
                            self._execute_worker, 
                            f_path, 
                            out_dir, 
                            f_settings,
                            is_batch=is_multi_task,          # 只要是多文件，一律套用不闪烁的全局总进度条
