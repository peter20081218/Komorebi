...
class FFmpegOrchestrator:
    ...
    def check_media_info(self, filepath):
        """利用 ffprobe 智能探测视频源文件属性，返回体积、视音频编码格式字典"""
        info = dict()
        try:
            size_mb = os.path.getsize(filepath) / (1024 * 1024)
            info.update({"size_mb": size_mb})
            
            cmd_v = tuple((self.ffprobe_cmd, "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=codec_name", "-of", "csv=p=0", filepath))
            res_v = subprocess.run(cmd_v, capture_output=True, text=True, shell=False)
            # 修复1：原视频若有多个视频流（如MPEG2VIDEO\nMPEG2VIDEO），只取第一行，防止破坏排版
            v_out = res_v.stdout.strip().split("\n").strip().replace(",", "") if res_v.stdout.strip() else "无/未知"
            info.update({"v_codec": v_out})
            
            cmd_a = tuple((self.ffprobe_cmd, "-v", "error", "-select_streams", "a:0", "-show_entries", "stream=codec_name", "-of", "csv=p=0", filepath))
            res_a = subprocess.run(cmd_a, capture_output=True, text=True, shell=False)
            a_out = res_a.stdout.strip().split("\n").strip().replace(",", "") if res_a.stdout.strip() else "无"
            info.update({"a_codec": a_out})
            
        except Exception:
            info.update({"size_mb": 0.0, "v_codec": "未知", "a_codec": "未知"})
        return info
    ...
    def build_video_compress_command(self, input_file, output_file, video_category, quality_level, target_vcodec, use_hw=True, ext_audio=None):
        ...
        if video_category == "1": # 动漫
            q_map_anime = dict()
            q_map_anime.update({"1": "18", "2": "21", "3": "25", "4": "28"})
            cq_val = q_map_anime.get(quality_level, "21")
            ...
            q_map_screen = dict()
            q_map_screen.update({"1": "20", "2": "24", "3": "28", "4": "32"})
            cq_val = q_map_screen.get(quality_level, "24")
            ...
            q_map_live = dict()
            q_map_live.update({"1": "17", "2": "20", "3": "24", "4": "27"})
            cq_val = q_map_live.get(quality_level, "20")
            if quality_level in ("1", "2"):
        ...
        # 修复2：如果目标是 H.264，不论软硬解都强制转为 8-bit (yuv420p)，防止10-bit源引发 H264 不支持的崩溃
    ...
    def build_format_factory_command(self, input_file, output_file, format_type, compress_level=None, use_hw=True, target_vcodec="h264"):
        """构建全生态格式化工厂，支持视频、音频及动图的多级压缩算法"""
        cmd = list()
        cmd.extend((self.ffmpeg_cmd, "-y", "-i", input_file))
        ...
        if format_type in audio_formats:
            ...
            self.print_guide("加载高画质动图提取算法...", "INFO")
            if format_type == "gif":
            ...
            if compress_level:
                q_map = dict()
                q_map.update({"1": "18", "2": "23", "3": "28", "4": "32"})
                cq_val = q_map.get(compress_level, "23")
    ...
    def diagnostic_error_translator(self, error_log):
        ...
        if "10 bit encode not supported" in error_log:
            return "显卡硬件编码引擎过于老旧或格式不支持，H.264不支持10位色深压制。"
    ...
    def _estimate_size(self, orig_mb, codec, level):
        """精准的非线性模型预测视频压缩后的体积损失"""
        base_ratios = dict()
        base_ratios.update({"h264": {"1": 0.70, "2": 0.45, "3": 0.25, "4": 0.15}})
        base_ratios.update({"hevc": {"1": 0.50, "2": 0.30, "3": 0.15, "4": 0.08}})
        base_ratios.update({"av1":  {"1": 0.40, "2": 0.20, "3": 0.10, "4": 0.05}})
        
        ratio_dict = base_ratios.get(codec, base_ratios.get("h264"))
        ratio = ratio_dict.get(level, 0.45)
        return str(round(orig_mb * ratio, 1))
    ...
    def run_interactive_ui(self):
        while True:
            ...
            self.print_guide("===========================================")
            print("请选择操作模式：\n  1. 场景化视频极致压缩 (动漫/录屏/拍摄独立算法)\n  2. 全能格式化工厂 (视频互转/纯音频压缩/动图生成)")
            ...
            print("\n-------------------------------------------")
            self.print_guide("源文件侦测结果: 体积 " + str(round(orig_mb, 2)) + " MB | 视频编码: " + media_info.get("v_codec").upper() + " | 音频编码: " + media_info.get("a_codec").upper(), "INFO")
            if len(self.available_hw_encoders) > 0:
                self.print_guide("系统支持的硬件编码器: " + ", ".join(self.available_hw_encoders).upper(), "SUCCESS")
            else:
            ...
            if choice == "1":
                ...
                # 修复3：保护性拦截 - WebM 容器严格不支持 H.264 和 HEVC
                if target_fmt == "webm" and target_vcodec in ("h264", "hevc"):
                    self.print_guide("【兼容性修正】WebM 容器仅支持 AV1/VP9 编码，已自动为您将编码器切换至 AV1。", "WARNING")
                    target_vcodec = "av1"
                
                print("\n请选择该场景下的压缩预设挡位 (系统已为您动态预估结果)：")
                print("  1. 极致高保真 (画质无损 | 预估将缩减至: ~" + self._estimate_size(orig_mb, target_vcodec, "1") + " MB)")
                print("  2. 优质归档   (肉眼极难分辨损失 | 预估将缩减至: ~" + self._estimate_size(orig_mb, target_vcodec, "2") + " MB)")
                print("  3. 日常流媒体 (轻微损失，高性价比 | 预估将缩减至: ~" + self._estimate_size(orig_mb, target_vcodec, "3") + " MB)")
                print("  4. 极限存储   (牺牲部分画质换取极限体积 | 预估将缩减至: ~" + self._estimate_size(orig_mb, target_vcodec, "4") + " MB)")
                ...
                if not has_audio:
                    self.print_guide("检测到该视频缺少音频轨道！", "WARNING")
                    add_audio = input("是否需要混入外部音轨？(y: 合并外部音频 / n: 保持纯无声画面, 默认n): ").strip().lower()
                    if add_audio == 'y':
                ...
                print("\n【支持的视频格式】: mp4, mkv, avi, mov, flv, wmv, webm, 3gp, ts 等")
                ...
                cmd = self.build_format_factory_command(input_path, output_path, target_fmt, compress_level=compress_level, use_hw=True, target_vcodec=target_vcodec)
            ...
            if success and os.path.exists(output_path):
                try:
                    ...
                    if reduction > 0:
                        ...
                        self.print_guide("▶ 压缩结果: 增大了 " + str(round(abs(reduction), 2)) + "% (源文件可能已是极限压缩状态或选择了高保真格式)", "WARNING")
                    print("===========================================\n")
                except Exception:
                    pass
